...
typedef NS_ENUM(NSUInteger, YMEncodingNSType) {
    YMEncodingTypeNSUnknown = 0,
    YMEncodingTypeNSString,
    YMEncodingTypeNSMutableString,
    YMEncodingTypeNSValue,
    YMEncodingTypeNSNumber,
    YMEncodingTypeNSDecimalNumber,
    YMEncodingTypeNSData,
    YMEncodingTypeNSMutableData,
    YMEncodingTypeNSDate,
    YMEncodingTypeNSURL,
    YMEncodingTypeNSArray,
    YMEncodingTypeNSMutableArray,
    YMEncodingTypeNSDictionary,
    YMEncodingTypeNSMutableDictionary,
    YMEncodingTypeNSSet,
    YMEncodingTypeNSMutableSet,
};

/// Get the Foundation class type from property info.
static force_inline YMEncodingNSType YMClassGetNSType(Class cls) {
    if (!cls)
        return YMEncodingTypeNSUnknown;
    if ([cls isSubclassOfClass:[NSMutableString class]])
        return YMEncodingTypeNSMutableString;
    if ([cls isSubclassOfClass:[NSString class]])
        return YMEncodingTypeNSString;
    if ([cls isSubclassOfClass:[NSDecimalNumber class]])
        return YMEncodingTypeNSDecimalNumber;
    if ([cls isSubclassOfClass:[NSNumber class]])
        return YMEncodingTypeNSNumber;
    if ([cls isSubclassOfClass:[NSValue class]])
        return YMEncodingTypeNSValue;
    if ([cls isSubclassOfClass:[NSMutableData class]])
        return YMEncodingTypeNSMutableData;
    if ([cls isSubclassOfClass:[NSData class]])
        return YMEncodingTypeNSData;
    if ([cls isSubclassOfClass:[NSDate class]])
        return YMEncodingTypeNSDate;
    if ([cls isSubclassOfClass:[NSURL class]])
        return YMEncodingTypeNSURL;
    if ([cls isSubclassOfClass:[NSMutableArray class]])
        return YMEncodingTypeNSMutableArray;
    if ([cls isSubclassOfClass:[NSArray class]])
        return YMEncodingTypeNSArray;
    if ([cls isSubclassOfClass:[NSMutableDictionary class]])
        return YMEncodingTypeNSMutableDictionary;
    if ([cls isSubclassOfClass:[NSDictionary class]])
        return YMEncodingTypeNSDictionary;
    if ([cls isSubclassOfClass:[NSMutableSet class]])
        return YMEncodingTypeNSMutableSet;
    if ([cls isSubclassOfClass:[NSSet class]])
        return YMEncodingTypeNSSet;
    return YMEncodingTypeNSUnknown;
}

/// Whether the type is c number.
static force_inline BOOL YMEncodingTypeIsCNumber(YMEncodingType type) {
    switch (type & YMEncodingTypeMask) {
        case YMEncodingTypeBool:
        case YMEncodingTypeInt8:
        case YMEncodingTypeUInt8:
        case YMEncodingTypeInt16:
        case YMEncodingTypeUInt16:
        case YMEncodingTypeInt32:
        case YMEncodingTypeUInt32:
        case YMEncodingTypeInt64:
        case YMEncodingTypeUInt64:
        case YMEncodingTypeFloat:
        case YMEncodingTypeDouble:
        case YMEncodingTypeLongDouble:
            return YES;
        default:
            return NO;
    }
...
static force_inline NSNumber *YMNSNumberCreateFromID(__unsafe_unretained id value) {
...
static force_inline NSDate *YMNSDateFromString(__unsafe_unretained NSString *string) {
    typedef NSDate * (^YMNSDateParseBlock)(NSString *string);
#define kParserNum 34
    static YMNSDateParseBlock blocks[kParserNum + 1] = {0};
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        {
            /*
...
if (string.length > kParserNum)
    return nil;
YMNSDateParseBlock parser = blocks[string.length];
if (!parser)
    return nil;
...
static force_inline Class YMNSBlockClass() {
    static Class cls;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        void (^block)(void) = ^{
...
/**
 ...
 length: 20/24/25
 */
static force_inline NSDateFormatter *YMISODateFormatter() {
...
static force_inline id YMValueForKeyPath(__unsafe_unretained NSDictionary *dic, __unsafe_unretained NSArray *keyPaths) {
    id value = nil;
...
static force_inline id YMValueForMultiKeys(__unsafe_unretained NSDictionary *dic,
    ...
    for (NSString *key in multiKeys) {
        if ([key isKindOfClass:[NSString class]]) {
            ...
            value = YMValueForKeyPath(dic, (NSArray *)key);
            if (value)
                break;
        }
    }
...
@interface _YMModelPropertyMeta : NSObject {
  @package
    NSString *_name;                         ///< property's name
    YMEncodingType _type;                    ///< property's type
    YMEncodingNSType _nsType;                ///< property's Foundation type
    ...
    YMClassPropertyInfo *_info;  ///< property's info
    _YMModelPropertyMeta *_next; ///< next meta if there are multiple properties mapped to the same key.
}
@end

@implementation _YMModelPropertyMeta
+ (instancetype)metaWithClassInfo:(YMClassInfo *)classInfo
                     propertyInfo:(YMClassPropertyInfo *)propertyInfo
    ...
    _YMModelPropertyMeta *meta = [self new];
    meta->_name = propertyInfo.name;
    meta->_type = propertyInfo.type;
    meta->_info = propertyInfo;
    meta->_genericCls = generic;

    if ((meta->_type & YMEncodingTypeMask) == YMEncodingTypeObject) {
        meta->_nsType = YMClassGetNSType(propertyInfo.cls);
    } else {
        meta->_isCNumber = YMEncodingTypeIsCNumber(meta->_type);
    }
    if ((meta->_type & YMEncodingTypeMask) == YMEncodingTypeStruct) {
        /*
    ...
    } else if (meta->_cls && meta->_nsType == YMEncodingTypeNSUnknown) {
    ...
    if (meta->_getter && meta->_setter) {
         ...
         */
        switch (meta->_type & YMEncodingTypeMask) {
            case YMEncodingTypeBool:
            case YMEncodingTypeInt8:
            case YMEncodingTypeUInt8:
            case YMEncodingTypeInt16:
            ...
            case YMEncodingTypeFloat:
            case YMEncodingTypeDouble:
            case YMEncodingTypeObject:
            case YMEncodingTypeClass:
            case YMEncodingTypeBlock:
            case YMEncodingTypeStruct:
            case YMEncodingTypeUnion: {
...
@interface _YMModelMeta : NSObject {
  @package
    YMClassInfo *_classInfo;
    /// Key:mapped key and key path, Value:_YMModelPropertyMeta.
    NSDictionary *_mapper;
    /// Array<_YMModelPropertyMeta>, all property meta of this model.
    NSArray *_allPropertyMetas;
    /// Array<_YMModelPropertyMeta>, property meta which is mapped to a key path.
    NSArray *_keyPathPropertyMetas;
    /// Array<_YMModelPropertyMeta>, property meta which is mapped to multi keys.
    NSArray *_multiKeysPropertyMetas;
    /// The number of mapped key (and key path), same to _mapper.count.
    NSUInteger _keyMappedCount;
    /// Model class type.
    YMEncodingNSType _nsType;
...
@implementation _YMModelMeta
- (instancetype)initWithClass:(Class)cls {
    YMClassInfo *classInfo = [YMClassInfo classInfoWithClass:cls];
    if (!classInfo)
        return nil;
    self = [super init];
    ...
    if ([cls respondsToSelector:@selector(modelPropertyBlacklist)]) {
        NSArray *properties = [(id<YMModel>)cls modelPropertyBlacklist];
        if (properties) {
    ...
    if ([cls respondsToSelector:@selector(modelPropertyWhitelist)]) {
        NSArray *properties = [(id<YMModel>)cls modelPropertyWhitelist];
        if (properties) {
    ...
    if ([cls respondsToSelector:@selector(modelContainerPropertyGenericClass)]) {
        genericMapper = [(id<YMModel>)cls modelContainerPropertyGenericClass];
        if (genericMapper) {
    ...
    YMClassInfo *curClassInfo = classInfo;
    while (curClassInfo &&
           curClassInfo.superCls != nil) { // recursive parse super class, but ignore root class (NSObject/NSProxy)
        for (YMClassPropertyInfo *propertyInfo in curClassInfo.propertyInfos.allValues) {
            ...
            if (whitelist && ![whitelist containsObject:propertyInfo.name])
                continue;
            _YMModelPropertyMeta *meta = [_YMModelPropertyMeta metaWithClassInfo:classInfo
    ...
    if ([cls respondsToSelector:@selector(modelCustomPropertyMapper)]) {
        NSDictionary *customMapper = [(id<YMModel>)cls modelCustomPropertyMapper];
        [customMapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *mappedToKey, BOOL *stop) {
            _YMModelPropertyMeta *propertyMeta = allPropertyMetas[propertyName];
            if (!propertyMeta)
                return;
    ...
    [allPropertyMetas
        enumerateKeysAndObjectsUsingBlock:^(NSString *name, _YMModelPropertyMeta *propertyMeta, BOOL *stop) {
...
static force_inline NSNumber *ModelCreateNumberFromProperty(__unsafe_unretained id model,
                                                            __unsafe_unretained _YMModelPropertyMeta *meta) {
    switch (meta->_type & YMEncodingTypeMask) {
        case YMEncodingTypeBool: {
...
static void ModelSetValueForProperty(__unsafe_unretained id model, __unsafe_unretained id value,
                                     __unsafe_unretained _YMModelPropertyMeta *meta) {
    if (meta->_isCNumber) {
        NSNumber *num = YMNSNumberCreateFromID(value);
        ...
        if (value == (id)kCFNull) {
            ...
            switch (meta->_nsType) {
                case YMEncodingTypeNSString:
                case YMEncodingTypeNSMutableString: {
                    if ([value isKindOfClass:[NSString class]]) {
                        if (meta->_nsType == YMEncodingTypeNSString) {
                        ...
                        ((void (*)(id, SEL, id))(void *)objc_msgSend)(
                            ...
                            (meta->_nsType == YMEncodingTypeNSString) ? ((NSNumber *)value).stringValue
                        ...
                        ((void (*)(id, SEL, id))(void *)objc_msgSend)(
                            (id)model, meta->_setter,
                            (meta->_nsType == YMEncodingTypeNSString) ? ((NSURL *)value).absoluteString
                                                                      : ((NSURL *)value).absoluteString.mutableCopy);
                    } else if ([value isKindOfClass:[NSAttributedString class]]) {
                        ((void (*)(id, SEL, id))(void *)objc_msgSend)(
                            (id)model, meta->_setter,
                            (meta->_nsType == YMEncodingTypeNSString)
                                ? ((NSAttributedString *)value).string
                                : ((NSAttributedString *)value).string.mutableCopy);
                    }
                } break;

                case YMEncodingTypeNSValue:
                case YMEncodingTypeNSNumber:
                case YMEncodingTypeNSDecimalNumber: {
                    if (meta->_nsType == YMEncodingTypeNSNumber) {
                        ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter,
                                                                      YMNSNumberCreateFromID(value));
                    } else if (meta->_nsType == YMEncodingTypeNSDecimalNumber) {
                ...
                case YMEncodingTypeNSData:
                case YMEncodingTypeNSMutableData: {
                    if ([value isKindOfClass:[NSData class]]) {
                        if (meta->_nsType == YMEncodingTypeNSData) {
                            ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter, value);
                        } else {
                        ...
                        if (meta->_nsType == YMEncodingTypeNSMutableData) {
                ...
                case YMEncodingTypeNSDate: {
                    if ([value isKindOfClass:[NSDate class]]) {
                        ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter, value);
                    } else if ([value isKindOfClass:[NSString class]]) {
                        ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter,
                                                                      YMNSDateFromString(value));
                    }
                } break;

                case YMEncodingTypeNSURL: {
                ...
                case YMEncodingTypeNSArray:
                case YMEncodingTypeNSMutableArray: {
                    if (meta->_genericCls) {
                        ...
                        if ([value isKindOfClass:[NSArray class]]) {
                            if (meta->_nsType == YMEncodingTypeNSArray) {
                                ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter, value);
                            } else {
                                ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter,
                ...
                case YMEncodingTypeNSDictionary:
                case YMEncodingTypeNSMutableDictionary: {
                    if ([value isKindOfClass:[NSDictionary class]]) {
                        if (meta->_genericCls) {
                            ...
                            if (meta->_nsType == YMEncodingTypeNSDictionary) {
                                ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter, value);
                            } else {
                                ((void (*)(id, SEL, id))(void *)objc_msgSend)((id)model, meta->_setter,
                ...
                case YMEncodingTypeNSSet:
                case YMEncodingTypeNSMutableSet: {
                    NSSet *valueSet = nil;
                    if ([value isKindOfClass:[NSArray class]])
                    ...
                    if (meta->_genericCls) {
                        ...
                        if (meta->_nsType == YMEncodingTypeNSSet) {
        ...
        switch (meta->_type & YMEncodingTypeMask) {
            ...
            case YMEncodingTypeBlock: {
                ...
                } else if ([value isKindOfClass:YMNSBlockClass()]) {
            ...
            case YMEncodingTypePointer:
            case YMEncodingTypeCString: {
                if (isNull) {
                    ((void (*)(id, SEL, void *))(void *)objc_msgSend)((id)model, meta->_setter, (void *)NULL);
...
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ...
    __unsafe_unretained _YMModelMeta *meta = (__bridge _YMModelMeta *)(context->modelMeta);
    __unsafe_unretained _YMModelPropertyMeta *propertyMeta = [meta->_mapper objectForKey:(__bridge id)(_key)];
...
/**
 ...
 @param _propertyMeta should not be nil, _YMModelPropertyMeta.
 @param _context      _context.model and _context.dictionary should not be nil.
 */
static void ModelSetWithPropertyMetaArrayFunction(const void *_propertyMeta, void *_context) {
    ModelSetContext *context = _context;
    __unsafe_unretained NSDictionary *dictionary = (__bridge NSDictionary *)(context->dictionary);
    __unsafe_unretained _YMModelPropertyMeta *propertyMeta = (__bridge _YMModelPropertyMeta *)(_propertyMeta);
    if (!propertyMeta->_setter)
        return;
    id value = nil;

    if (propertyMeta->_mappedToKeyArray) {
        value = YMValueForMultiKeys(dictionary, propertyMeta->_mappedToKeyArray);
    } else if (propertyMeta->_mappedToKeyPath) {
        value = YMValueForKeyPath(dictionary, propertyMeta->_mappedToKeyPath);
    } else {
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    if ([model isKindOfClass:[NSDate class]])
        return [YMISODateFormatter() stringFromDate:(id)model];
    if ([model isKindOfClass:[NSData class]])
        return nil;

    _YMModelMeta *modelMeta = [_YMModelMeta metaWithClass:[model class]];
    ...
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey,
                                                            _YMModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        if (propertyMeta->_isCNumber) {
            ...
            switch (propertyMeta->_type & YMEncodingTypeMask) {
                case YMEncodingTypeObject: {
                    id v = ((id(*)(id, SEL))(void *)objc_msgSend)((id)model, propertyMeta->_getter);
                    value = ModelToJSONObjectRecursive(v);
                    if (value == (id)kCFNull)
    ...
    if (modelMeta->_hasCustomTransformToDictionary) {
        BOOL suc = [((id<YMModel>)model) modelCustomTransformToDictionary:dic];
        if (!suc)
            return nil;
    }
    return result;
    ...
    switch (modelMeta->_nsType) {
        ...
        default: {
            ...
            NSArray *properties = [modelMeta->_allPropertyMetas
                sortedArrayUsingComparator:^NSComparisonResult(_YMModelPropertyMeta *p1, _YMModelPropertyMeta *p2) {
                    return [p1->_name compare:p2->_name];
                }];

            [desc appendFormat:@" {\n"];
            for (NSUInteger i = 0, max = properties.count; i < max; i++) {
                _YMModelPropertyMeta *property = properties[i];
                NSString *propertyDesc;
                if (property->_isCNumber) {
                    NSNumber *num = ModelCreateNumberFromProperty(model, property);
                    propertyDesc = num.stringValue;
                } else {
                    switch (property->_type & YMEncodingTypeMask) {
                        case YMEncodingTypeObject: {
...
+ (instancetype)pa_modelWithDictionary:(NSDictionary *)dictionary {
    ...
    _YMModelMeta *modelMeta = [_YMModelMeta metaWithClass:cls];
...
- (BOOL)pa_modelSetWithDictionary:(NSDictionary *)dic {
    ...
    _YMModelMeta *modelMeta = [_YMModelMeta metaWithClass:object_getClass(self)];
    if (modelMeta->_keyMappedCount == 0)
        return NO;

    if (modelMeta->_hasCustomWillTransformFromDictionary) {
        dic = [((id<YMModel>)self) modelCustomWillTransformFromDictionary:dic];
        if (![dic isKindOfClass:[NSDictionary class]])
            return NO;
    }
    ...
    if (modelMeta->_hasCustomTransformFromDictionary) {
        return [((id<YMModel>)self) modelCustomTransformFromDictionary:dic];
    }
    return YES;
}
...
- (id)pa_modelCopy {
    ...
    for (_YMModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
...
- (id)pa_modelInitWithCoder:(NSCoder *)aDecoder {
    ...
    for (_YMModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
        ...
        if (propertyMeta->_isCNumber) {
            ...
            YMEncodingType type = propertyMeta->_type & YMEncodingTypeMask;
            switch (type) {
                case YMEncodingTypeObject: {
